...
public class FilterUserRepositoryImpl implements FilterUserRepository {
    ...
    private final EntityManager entityManager; // - Поле типа EntityManager используется для взаимодействия с контекстом персистентности и выполнения операций с базой данных.
    ...
    public List<User> findAllByFilter(UserFilter filter) {
        ...
        return new JPAQuery<User>(entityManager)
                ...
                .from(user)
                .where(predicate)
                .fetch(); // выполняет запрос и возвращает список пользователей.
    }
